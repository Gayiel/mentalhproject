...
You've shown real courage in sharing your feelings today. How are you doing right now?`; this.addHumanMessage(intro); },2000); }
  ...
  notifyHuman(message, isCrisis){ if (isCrisis){ const flagged=document.getElementById('flagged-list'); if (flagged){ const item=document.createElement('div'); item.className='flag-item'; item.innerHTML = `<strong>Flagged</strong>: ${this.escapeHtml(message)}<div class="meta-row"><span>${new Date().toLocaleTimeString()}</span><span>Distress ${this.getLastScore()}</span></div>`; flagged.prepend(item); } } }
  ...
  logAction(type, data={}){ const entry={type, ts:Date.now(), ...data}; const key='mf_session_actions'; try{ const prev=JSON.parse(sessionStorage.getItem(key)||'[]'); prev.push(entry); sessionStorage.setItem(key, JSON.stringify(prev)); }catch(e){} }
